...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'byggtjansthammaro-leads' ),
					'singular_name' => __( 'Lead', 'byggtjansthammaro-leads' ),
					'edit_item'     => __( 'Förfrågan', 'byggtjansthammaro-leads' ),
					'search_items'  => __( 'Search leads', 'byggtjansthammaro-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'byggtjansthammaro-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'byggtjansthammaro-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'byggtjansthammaro-leads' ),
			'npl_phone'    => __( 'Phone', 'byggtjansthammaro-leads' ),
			'npl_service'  => __( 'Needs', 'byggtjansthammaro-leads' ),
			'npl_timeline' => __( 'When', 'byggtjansthammaro-leads' ),
			'npl_zip'      => __( 'ZIP', 'byggtjansthammaro-leads' ),
			'npl_source'   => __( 'Source', 'byggtjansthammaro-leads' ),
			'npl_status'   => __( 'Status', 'byggtjansthammaro-leads' ),
			'date'         => __( 'Received', 'byggtjansthammaro-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'byggtjansthammaro-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'byggtjansthammaro-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'byggtjansthammaro-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'byggtjansthammaro-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'byggtjansthammaro-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'byggtjansthammaro-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'byggtjansthammaro-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'byggtjansthammaro-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'byggtjansthammaro-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'byggtjansthammaro-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'byggtjansthammaro-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'byggtjansthammaro-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
