 ...
 * or photo. They call by_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Byggtjänst Hammarö AB.
 *
 ...
 * @return string
 */
function by_sample_note() {
	return by_lead( 'ar_riktig' ) ? '' : ' <span class="by-sample">(platshållare)</span>';
}
 ...
 * @return string File name for by_lead_img().
 */
function by_lead_bild( $i ) {
	$bilder = (array) by_lead( 'bilder', array() );
 ...
 * @return string
 */
function by_ring_label() {
	$namn = by_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, by_lead( 'telefon' ) ) : 'Ring ' . by_lead( 'telefon' );
}
